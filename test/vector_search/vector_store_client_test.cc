...
SEASTAR_TEST_CASE(vector_store_client_test_ann_request) {
    ...
    co_await do_with_cql_env(
            [&server](cql_test_env& env) -> future<> {
                ...
                BOOST_REQUIRE_EQUAL(server->ann_requests().back().body, R"({"vector":[0.3,0.2,0.1],"limit":1,"filter":{}})");
                ...
                BOOST_REQUIRE(!server->ann_requests().empty());
                BOOST_REQUIRE_EQUAL(server->ann_requests().back().body, R"({"vector":[0.1,0.2,0.3],"limit":2,"filter":{}})");
